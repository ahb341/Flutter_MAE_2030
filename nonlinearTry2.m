...
for i = 1:length(Kh_iter)
    for j = 1:length(Ch_iter)
      for k = 1:length(Ca_iter)
        ...
        fprintf('generated fig %d %d %d',i,j,k)
        
        filename = ['figure' '-' num2str(Kh_iter(i)) '-' num2str(Ch_iter(j)) '-' num2str(Ca_iter(k)) '.jpg'];
        saveas(1,filename)
        fprintf('saved fig %d %d %d',Kh_iter(i),Ch_iter(j),Ca_iter(k))
